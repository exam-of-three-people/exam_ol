...
tests = {"choice_question": [
    ...
    {"question": "数据库系统的数据独立性体现在__。", "A": "不会因为数据的变化而影响到应用程序", "B": "不会因为数据存储结构与数据逻辑结构的变化而影响应用程序",
     "C": "不会因为存储策略的变化而影响存储结构", "D": "不会因为某些存储结构的变化而影响其他的存储结构", "answer": "B"},
    {"question": "关系数据模型是目前最重要的一种数据模型，它的三个要素分别是__。", "A": "实体完整性、参照完整性、用户自定义完整性", "B": "数据结构、关系操作、完整性约束",
     "C": "数据增加、数据修改、数据查询", "D": "外模式、模式、内模式", "answer": "B"},
    ...
    {"question": "下面列出的数据库管理技术发展的三个阶段中，没有专门的软件对数据进行管理的是___。 I．人工管理阶段 II．文件系统阶段 III．数据库阶段", "A": " I 和 II", "B": "只有 II",
     "C": " II 和 II", "D": "只有 I", "answer": "D"},
    ...
    "true_false_question": [
        ...
        {"question": "对于一个基本关系表来说，列的顺序无所谓——即改变属性的排列顺序不会改变该关系的本质结构。（   ）", "answer": "1"},
        {"question": "对于一个基本关系表来说，行的顺序无所谓——即将一条记录插入在第一行和插入在第五行没有本质上的不同。（   ）", "answer": "1"},
        {"question": "在一个关系表上最多只能建立一个聚簇索引。（  ）", "answer": "1"},
        {"question": "若.一个数据库管理系统提供了强制存取控制机制（MAC），则它一定也会提供自主存取控制机制。（  ）", "answer": "1"},
